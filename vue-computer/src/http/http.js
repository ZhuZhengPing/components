...
import {GetToken} from '@/http/index.js';
...
async function send(url,data,method){
    data = data||{};
    ...
    return axios({
        ...
        if (error && error.response && error.response.data && error.response.data.ExceptionMessage) {
            ElMessage.error(error.response.data.ExceptionMessage);
        } 
...
//     return send('POST', url, null, data);
// }
